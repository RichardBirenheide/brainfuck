...
//		System.out.println(element);
		if (element instanceof BfBreakpoint) {
			...
			try {
				...
				DbgActivator.getDefault().logError("Marker Resource", e);
			}
		}
			...
			try {
				...
				DbgActivator.getDefault().logError("Name could not be retrieved", ex);
			}
		}
		return null;
	}
...
//		String text = preference.getPreferenceLabel();
		...
		if (frame instanceof BfStackFrame) {
			try {
				...
				DbgActivator.getDefault().logError("Instruction Pointer location could not be revealed", ex);
			}
		}
		return text;
	}
